<template>
  <div class="p-2">
        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            <el-form-item label="操作模块" prop="moduleName">
              <el-input v-model="queryParams.moduleName" placeholder="请输入操作模块" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="操作方法" prop="methodName">
              <el-input v-model="queryParams.methodName" placeholder="请输入操作方法" clearable @keyup.enter="handleQuery" />
            </el-form-item>
    ...
    <el-card shadow="hover">
      ...
      <el-table v-loading="loading" border :data="operLogList" @selection-change="handleSelectionChange">
...
import { list, delOperLog, exportOperLog } from '@/api/system/operlog';
import { OperLogForm, OperLogQuery } from '@/api/system/operlog/types';
...
function getList() {
  ...
  list(query)
    .then((response) => {
      operLogList.value = response.data.records || response.data.rows || [];
...
function handleDelete(row?: any) {
  ...
  ElMessageBox.confirm('是否确认删除日志编号为"' + deleteIds + '"的数据项？', '警告', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  })
    .then(() => {
      return delOperLog(Array.isArray(deleteIds) ? deleteIds : [deleteIds]);
    })
    .then(() => {
      getList();
...
function handleExport() {
  ElMessageBox.confirm('是否确认导出所有操作日志数据项?', '警告', {
    ...
    .then((response) => {
      proxy.download(response, 'operlog_' + new Date().getTime() + '.xlsx');
    })
    .catch(() => {});
}
