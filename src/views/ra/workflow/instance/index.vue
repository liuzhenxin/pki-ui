<template>
  <div class="app-container ra-workflow-instance-page">
    ...
    <el-table v-loading="loading" :data="taskList" border>
      ...
      <el-table-column label="审批人" prop="assigneeName" min-width="120" show-overflow-tooltip />
      <el-table-column label="审批意见" prop="comment" min-width="180" show-overflow-tooltip>
        <template #default="{ row }">{{ row.comment || '-' }}</template>
      </el-table-column>
      <el-table-column label="处理时间" prop="taskEndTime" align="center" width="170">
        <template #default="{ row }">{{ parseTime(row.taskEndTime) }}</template>
      </el-table-column>
      <el-table-column label="状态" align="center" width="100">
        <template #default="{ row }">
          <el-tag type="success">{{ row.taskStatusName }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="detailOpen" title="已办记录详情" width="720px" append-to-body>
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="审批人">{{ detail.assigneeName || '-' }}</el-descriptions-item>
        ...
        <el-descriptions-item label="处理时间">{{ parseTime(detail.taskEndTime) || '-' }}</el-descriptions-item>
      </el-descriptions>
    </el-dialog>
  </div>
</template>

<script setup name="RaWorkflowInstance" lang="ts">
import { getRaWorkflowTask, pageRaWorkflowDone, RaWorkflowTask } from '@/api/ra/workflowTask';
...
async function getList() {
  loading.value = true;
  try {
    const res = await pageRaWorkflowDone({
      ...queryParams,
